...
std::unique_ptr<Model> DirectX::Model::CreateFromCMO(ID3D11Device* d3dDevice, const uint8_t* meshData, size_t dataSize, IEffectFactory& fxFactory, bool ccw, bool pmalpha)
{
	...
	for (UINT meshIndex = 0; meshIndex < *nMesh; ++meshIndex)
	{
		...
		if (*bSkeleton)
		{
			...
			for (UINT j = 0; j < *nBones; ++j)
			{
				...
				//bones->InvBindPos is equal to Invert(bones->BindPos);
				ModelMesh::Bone cmoBone(boneName, bones->ParentIndex, bones->InvBindPos, bones->BindPos, bones->LocalTransform);
				mesh->m_bones.emplace_back(cmoBone);
			}

			mesh->m_animationTransformed.resize(mesh->m_bones.size());
			...
			for (UINT j = 0; j < *nClips; ++j)
			{
				...
				mesh->m_animationClips[clipName].StartTime = clip->StartTime;
				mesh->m_animationClips[clipName].EndTime = clip->EndTime;

				ModelMesh::KeyframeArray & keyFrames = mesh->m_animationClips[clipName].Keyframes;

				keyFrames.reserve(clip->keys);

				for (UINT k = 0; k < clip->keys; k++)
				{
					auto keys = reinterpret_cast<const ModelMesh::Keyframe*>(meshData + usedSize);
					usedSize += sizeof(ModelMesh::Keyframe);
					if (dataSize < usedSize)
						throw std::exception("End of file");
		...
		for (UINT j = 0; j < *nMats; ++j)
		{
			...
			if (fxFactoryDGSL)
			{
				...
				info.normalTexture = m.texture[1].empty() ? nullptr : m.texture[1].c_str();
	...
	std::unique_ptr<Model> temp(model.release());
