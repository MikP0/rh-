	...
	_Inout_ IEffectFactory& fxFactory, _Inout_ MaterialRecordSDKMESH& m)
{
	...
	info.diffuseTexture = txtName;
	info.normalTexture = txtName2;
...
std::unique_ptr<Model> DirectX::Model::CreateFromSDKMESH(ID3D11Device* d3dDevice, const uint8_t* meshData, size_t dataSize, IEffectFactory& fxFactory, bool ccw, bool pmalpha)
{
	...
	if (dataSize < header->FrameDataOffset
		...
		throw std::exception("End of file");
	// TODO - 
	auto frameArray = reinterpret_cast<const DXUT::SDKMESH_FRAME*>(meshData + header->FrameDataOffset);
	...
	for (UINT meshIndex = 0; meshIndex < header->NumMeshes; ++meshIndex)
	{
		...
		mesh->NumFrameInfluences = mh.NumFrameInfluences;

		//Frame influences
		mesh->frameInfluences.reserve(mh.NumFrameInfluences);
		auto influences = reinterpret_cast<const UINT*>(meshData + mh.FrameInfluenceOffset);
		for (UINT j = 0; j < mh.NumFrameInfluences; j++)
		{
			mesh->frameInfluences.emplace_back(influences[j]);
		}
		...
		for (UINT j = 0; j < mh.NumSubsets; ++j)
		{
			...
			auto & vh = vbArray[mh.VertexBuffers[0]];
			auto verts = reinterpret_cast<const uint8_t*>(bufferData + (vh.DataOffset - bufferDataOffset));

			auto & ih = ibArray[mh.IndexBuffer];
			auto ibs32 = reinterpret_cast<const uint32_t*>(bufferData + ih.DataOffset - bufferDataOffset);
			auto ibs16 = reinterpret_cast<const uint16_t*>(bufferData + ih.DataOffset - bufferDataOffset);

			for (uint32_t index = part->startIndex; index < part->startIndex + part->indexCount; ++index)
			{
				uint32_t currentIndex = 0;

				if (part->indexFormat == DXGI_FORMAT_R16_UINT)
				{
					currentIndex = ibs16[index];
				}
				else
				{
					currentIndex = ibs32[index];
				}

				XMFLOAT3* pt = (XMFLOAT3*)(&verts[currentIndex*part->vertexStride]);
		...
		XMVECTOR half = XMVectorSubtract(u, l); ////////////////////// *0.5;  !!!!!!!!!!!!!1111
		XMVECTOR center = XMVectorAdd(l, half);

		XMStoreFloat3(&mesh->boundingBox.Extents, half);
		XMStoreFloat3(&mesh->boundingBox.Center, center);
		BoundingSphere::CreateFromBoundingBox(mesh->boundingSphere, mesh->boundingBox);
	...
	//create frame
	model->frames.reserve(header->NumFrames);

	for (UINT frameIndex = 0; frameIndex < header->NumFrames; ++frameIndex)
	{
		auto & sdkFrame = frameArray[frameIndex];

		auto frame = std::make_shared<ModelFrame>();

		WCHAR frameName[DXUT::MAX_FRAME_NAME];
		MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, sdkFrame.Name, -1, frameName, DXUT::MAX_FRAME_NAME);

		frame->name = frameName;
		frame->Mesh = sdkFrame.Mesh;
		frame->ParentFrame = sdkFrame.ParentFrame;
		frame->ChildFrame = sdkFrame.ChildFrame;
		frame->SiblingFrame = sdkFrame.SiblingFrame;
		frame->Matrix = sdkFrame.Matrix;
		frame->AnimationDataIndex = sdkFrame.AnimationDataIndex;

		model->frames.emplace_back(frame);
	}
...
bool DirectX::Model::LoadAnimation(const wchar_t* szFileName)
{
	using namespace DXUT;
	using namespace std;
	...
	if (FAILED(hr))
	{
		DebugTrace("LoadAnimation failed (%08X) loading '%ls'\n", hr, szFileName);
		throw std::exception("CreateFromSDKMESH");
		return false;
	}

	auto header = reinterpret_cast<const SDKANIMATION_FILE_HEADER*>(data.get());
	auto sdkAnimFrameData = reinterpret_cast<const SDKANIMATION_FRAME_DATA*>(data.get() + header->AnimationDataOffset);

	shared_ptr<Animation> anim(new Animation);

	// set frame transform type
	anim->frameTransformType = (FRAME_TRANSFORM_TYPE)header->FrameTransformType;

	anim->animFrames.reserve(header->NumFrames);
	UINT64 BaseOffset = sizeof(SDKANIMATION_FILE_HEADER);
	SDKANIMATION_DATA* pPreAnimData = NULL;

	for (UINT i = 0; i < header->NumFrames; i++)
	{
		SDKANIMATION_DATA* pAnimationData = (SDKANIMATION_DATA*)(data.get() +
			sdkAnimFrameData[i].DataOffset +
			BaseOffset);

		WCHAR frameName[DXUT::MAX_FRAME_NAME];
		MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, sdkAnimFrameData[i].FrameName, -1, frameName, DXUT::MAX_FRAME_NAME);

		auto animFrame = std::make_shared<AnimationFrame>();
		animFrame->frameName = frameName;

		if (pPreAnimData)
		{
			assert(UINT(pAnimationData - pPreAnimData) == header->NumAnimationKeys);
		}

		animFrame->animFrameDatas.resize(header->NumAnimationKeys);
		for (UINT j = 0; j < header->NumAnimationKeys; j++)
		{
			animFrame->animFrameDatas[j].Orientation = pAnimationData[j].Orientation;
			animFrame->animFrameDatas[j].Scaling = pAnimationData[j].Scaling;
			animFrame->animFrameDatas[j].Translation = pAnimationData[j].Translation;
		}

		pPreAnimData = pAnimationData;

		auto frame = std::find_if(begin(frames), end(frames), [=](shared_ptr<ModelFrame> Frame)
		{
			return animFrame->frameName == Frame->name ? true : false;
		});
		...
		anim->animFrames.emplace_back(animFrame);
	}

	wstring temp(szFileName), name;
	size_t beg = temp.find_last_of(L'\\');
	size_t end = temp.find_last_of(L'.');

	if (beg != -1 && end != -1)
		name = wstring(temp.begin() + beg + 1, temp.begin() + end);
	else
		name = szFileName;

	anim->animFPS = header->AnimationFPS;
	anim->numAnimKeys = header->NumAnimationKeys;
	anim->numBeginKey = 0;
