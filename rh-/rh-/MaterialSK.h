...
class MaterialSK : public RTTI
{
	RTTI_DECLARATIONS(MaterialSK, RTTI)

public:
	MaterialSK();
	MaterialSK(const std::string& defaultTechniqueName);
	virtual ~MaterialSK();

	Variable* operator[](const std::string& variableName);
	EffectSK* GetEffect() const;
	...
	virtual void Initialize(EffectSK& effect);
	virtual void CreateVertexBuffer(ID3D11Device* device, const ModelSK& model, std::vector<ID3D11Buffer*>& vertexBuffers) const;
	virtual void CreateVertexBuffer(ID3D11Device* device, const MeshSK& mesh, ID3D11Buffer** vertexBuffer) const = 0;
	virtual UINT VertexSize() const = 0;

protected:
	MaterialSK(const MaterialSK& rhs);
	MaterialSK& operator=(const MaterialSK& rhs);
	...
	virtual void MaterialSK::CreateInputLayout(Pass& pass, D3D11_INPUT_ELEMENT_DESC* inputElementDescriptions, UINT inputElementDescriptionCount);
